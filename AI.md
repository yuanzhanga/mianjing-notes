...
- 单次 MR 或工作区变更很大时，需要合理切分（按文件/按模块）、控制单次审查 token 与耗时
  ...
  先让AI分析依赖关系，生成变更说明书/全局上下文，再根据说明书进行分桶分agent审查

### 心得

- 提示词：给上下文、定约束、要完整

  技术实现，风险和边界，还有不确定的地方问我，最后给出详细的实施计划
